...
font = FontProperties(fname=r"D:\新建文件夹\beibai-4.txt", size=14)
...
class MyFrame(wx.Frame):
    def __init__(self, parent, id):
        ...
        # 创建文本，左对齐
        self.title = wx.StaticText(panel, label="1、beibao1.txt    2、beibao2.txt    3、beibao3.txt    4、beibao4.txt\n")
    ...
    def OnclickSubmit(self,event):
        ...
        if self.text_file.GetValue() == '':    # 判断文件序号是否为空
            ...
            global i
            if (fileNum==1):
                with open('beibao1.txt') as f:
                    for line[i] in f:
                        i=i+1
            if (fileNum==2):
                with open('beibao2.txt') as f:
                    for line[i] in f:
                        i=i+1
            if (fileNum==3):
                with open('beibao3.txt') as f:
                    for line[i] in f:
                        i=i+1
            if (fileNum==4):
                with open('beibao4.txt') as f:
                    for line[i] in f:
                        i=i+1
            message = '读取成功'
        else:
